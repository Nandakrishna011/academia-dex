  ...
  return (
    ...
    >
      <div className="w-full max-w-sm space-y-2 flex flex-col">
        ...
        {/* Banner */}
        <div 
          className="border rounded-lg p-2 flex items-start justify-between gap-2 text-xs"
          style={{ backgroundColor: "rgba(10, 10, 10, 0.45)", borderColor: "color-mix(in srgb, var(--accent) 30%, rgba(255,255,255,0.1))" }}
        >
          <div>
          ...
          <button
            ...
            className="shrink-0 px-2 py-1 rounded text-[11px] font-semibold"
        ...
        {/* Modal */}
        <div 
          className="w-full rounded-xl flex flex-col shadow-[0_8px_32px_rgba(0,0,0,0.37)] backdrop-blur-md overflow-hidden max-h-[70vh] border"
          style={{ backgroundColor: "rgba(10, 10, 10, 0.6)", borderColor: "color-mix(in srgb, var(--accent) 30%, rgba(255, 255, 255, 0.1))" }}
        >

          {/* Header */}
          <div className="px-3 py-2 border-b flex justify-between items-center text-sm" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
            <button
            ...
            <h2 className="font-semibold" style={{ color: "var(--text)" }}>{currentMonthName}</h2>
          ...
          <div className="overflow-y-auto px-3 py-2 space-y-2">
              ...
              {monthData.map((d) => {
                ...
                return (
                  <button
                    ...
                    className={`p-1 rounded-md flex flex-col items-center transition-all border border-transparent
                      ${isSelected ? "scale-105 shadow-sm" : ""}
                      ${!isSelected && !isPast ? "hover:opacity-80" : ""}
                      ${isPast ? "opacity-30" : ""}
                      ${!isSelected && isHoliday && !isPast ? "opacity-60" : ""}
                    `}
                    style={{
                      backgroundColor: isSelected ? "var(--accent)" : (!isPast && isHoliday) ? "rgba(255,200,0,0.1)" : (!isPast) ? "rgba(255,255,255,0.05)" : "transparent",
            ...
            {/* Prediction */}
            <div className="p-2 rounded-lg mt-1" style={{ backgroundColor: "rgba(0,0,0,0.2)" }}>
              <h3 className="font-medium text-sm mb-2" style={{ color: "var(--text)" }}>Prediction</h3>

              {result.length === 0 && (
                <p className="text-xs" style={{ color: "var(--muted)" }}>Select dates</p>
              )}
              ...
              <div className="space-y-2">
                {result.map((r) => {
                  ...
                  return (
                    <div
                      key={r.id}
                      className="rounded-lg p-2 border flex justify-between text-xs"
                      style={{ backgroundColor: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.1)" }}
                    >
                      <div>
                        <div className="font-medium" style={{ color: "var(--text)" }}>{d.title}</div>
                        <div style={{ color: "var(--muted)" }}>{d.percentage}%</div>
                      </div>

                      <div className="text-right">
                        {isDanger ? (
                          <div className="text-red-400">Req: {d.required}</div>
                        ) : (
                          <div className="text-green-400">Mar: {d.margin}</div>
                        )}

                        <div className={`${d.diff < 0 ? "text-red-400" : "text-green-400"}`}>
          ...
          {/* Footer */}
          <div className="p-2 border-t flex gap-2" style={{ borderColor: "rgba(255,255,255,0.1)", backgroundColor: "rgba(0,0,0,0.2)" }}>
            <button
              onClick={() => { onApply(result); onClose(); }}
              className="flex-1 py-2 rounded-md text-sm transition font-semibold"
              style={{ backgroundColor: "var(--accent)", color: "var(--accent-fg)" }}
            >
              Done
            </button>
            ...
            <button
              ...
              style={{ backgroundColor: "rgba(255,255,255,0.05)", color: "var(--text)", border: "1px solid rgba(255,255,255,0.1)" }}
            >
              Cancel
            </button>
          </div>
